...
echo -n "GET : "
FILENAME=$(date +%s%N).html
echo curl --max-time 1 --path-as-is -skg -o "$FILENAME" "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -H "$user_agent" -X GET
code=$(curl --max-time 1 --path-as-is -skg -o "$FILENAME" "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -H "$user_agent" -X GET)
print
echo -n "POST : "
FILENAME=$(date +%s%N).html
