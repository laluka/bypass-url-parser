...
if [[ -z "$URL" ]];
then
    echo "Usage: $0 --url <URL> [--outdir <OUTDIR>] [--header 'x-foo: bar']"
...
echo "HEADER=$HEADER"
IFS=DOESNOTEXISTS
HEADER_OPT=$( [[ -n "$HEADER" ]] && printf %s -H "$HEADER" )
echo "HEADER_OPT=$HEADER_OPT"
# Example : curl -kig http://127.0.0.1/foo $( [[ -n "$HEADER" ]] && printf %s -H "$HEADER" )
...
echo curl --max-time 1 "$HEADER_OPT" --path-as-is -skg -o "$FILENAME" -w "Status: %{http_code}, Length: %{size_download}\n" -X GET "$URL" -H "$user_agent"
code=$(curl --max-time 1 "$HEADER_OPT" --path-as-is -skg -o "$FILENAME" -w "Status: %{http_code}, Length: %{size_download}\n" -X GET "$URL" -H "$user_agent")
print
...
echo -n "GET : "
FILENAME=$(date +%s%N).html
echo curl --max-time 1 "$HEADER_OPT" --path-as-is -skg -o "$FILENAME" "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -H "$user_agent" -X GET
code=$(curl --max-time 1 "$HEADER_OPT" --path-as-is -skg -o "$FILENAME" "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -H "$user_agent" -X GET)
print
echo -n "POST : "
FILENAME=$(date +%s%N).html
