...
echo curl --path-as-is -skg -o "$FILENAME" -w "Status: %{http_code}, Length: %{size_download}\n" -X GET "$URL" -H "$user_agent"
code=$(curl --path-as-is -skg -o "$FILENAME" -w "Status: %{http_code}, Length: %{size_download}\n" -X GET "$URL" -H "$user_agent")
print
...
echo -n "GET : "
FILENAME=$(date +%s%N).html
echo curl --path-as-is -skg -o "$FILENAME" "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -H "$user_agent" -X GET
code=$(curl --path-as-is -skg -o "$FILENAME" "$URL" -w "Status: %{http_code}, Length: %{size_download}\n" -L -H "$user_agent" -X GET)
print
echo -n "POST : "
FILENAME=$(date +%s%N).html
